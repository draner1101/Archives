  ...
  //ADD REGION
	...
	function addRegionEquipe(id){
		...
		if (AS == 'entraineurs'){
			...
			newFirstSelect.setAttribute("onchange","fetch_position(this.value,"+value+","+id+")"); //Modification fait par Cédric Paquette
			newFirstSelect.className = "form-control";
		}
...
?>
<!-- Fait par Cédric Paquette Début, CSS modifier par Frédérick Turner-->
...
Adam Grenon
*/
			echo '<img  src="/Diablos_Archive/Diablos_en_fusion/Site/Images/default.png" style="visibility: visible;"  id="img_photoDefault" class="formulaire_photo" alt="Photo introuvable" />  <br>';
		...
		echo '<div>';
			...
			if ($_SESSION['AS'] == 'equipes'){
				...
				echo '<select name="id_sport" id="0" style="width: initial;height:30px;margin-bottom:5px" class="form-control" required>
					<option value="" selected disabled>Sélectionner un sport</option>';
				...
				if ($row[0] !== 'photo_profil'){ // and $row[0] !== 'note'){
					...
					$maskedClass = setMasked($row[0]);
						...
						if ($row[0] == 'sexe'){
							...
							//Fait par Cédric Paquette Début - Updated by Adam 31 mai
							elseif ($row[0] == 'taille'){
								...
								echo '<input id="grandeur" type="radio" name="grandeur" value="cm" onclick="showhide()" checked></input> Centimètres (cm)	<input id="grandeur" type="radio" name="grandeur" value="pi" onclick="showhide()"></input> Pieds (pi)</td></div>';
							}
							elseif ($row[0] == 'poids'){
							...
							elseif ($row[0] == 'note'){ //note equipe 
								...
								echo '<td><label>'.$v_new.'</label></td><td><input name="'.$row[0].'" id="'.$ctr.'" title="'.$v_new.'" class="form-control LookupFiltre " style="margin-top:5px;width:auto;" type="text"/></td>';
							}
							$ctr = $ctr + 1;
						}
						else{
			...
			echo '<div id="full_div_equipe" style="display:inline-block;">';
					echo '<a onclick="addRegionEquipe(0)" class="btn formulaire_button" style="float:right;font-size:18px;margin:5px;">+</a>';
				if ($_GET['MODE'] == 'aj'){
...
//Modification fait par Vincent Robert et Cédric Paquette Début
    								echo '<select name="id_equipe" id="id_equipe" style="display:none;width:35%;height:30px;float:left;" onchange="fetch_position(this.value,\'0\',\'0\')" class="form-control">
    								...
    								echo '</select>';	
								if ($_GET['MODE'] == 'aj'){
									echo "<script type='text/javascript'>addRegionEquipe(0);</script>";
								}
//Modification fait par Vincent Robert et Cédric Paquette Fin
...
//  								
//  							//Position
//  								echo '<select name="id_position" id="id_position" style="display:initial;width:35%;margin-bottom:5px;height:30px;float:left" class="form-control">
...
//  					//echo '</div>';
	...
	elseif ($_GET['MODE'] == 'mod'){
		try {
				...
				//Transfert DATA
					...
					foreach ($_SESSION['arrayChamps'] as $key=>$value){							
						if ($_SESSION['arrayChamps'][$conteur] == 'taille') {
							...
							echo '<script>changeHTML("taillecm","'.$array[$key].'")</script>';
						...
						elseif ($_SESSION['arrayChamps'][$conteur] == 'poids') {
							//$new_value = str_replace("_","'",$array[$key]);
							echo '<script>changeHTML("poids","'.$array[$key].'")</script>';
						}
						//Fait par Cédric Paquette Fin
						else {
							if ((is_null($array[$key])) or ($array[$key] == 'null') or ($array[$key]=='')){
								...
								//$new_value = str_replace("_","'",$array[$key]);
								echo '<script>changeHTML('.$key.',"'.$array[$key].'")</script>';
							}
						}
						$conteur = $conteur + 1;
					}
						...
						if ($row[0] > 0){
							for ($k = 1;$k<$row[0]+1;$k++){
								...
								foreach ($infoEquipe as $data){
									...
									if ($k == 0){
										echo '<script type="text/javascript">addRegionEquipe('.$listInfoEquipe[1].');</script>'; //Modification fait par Vincent Robert et Cédric Paquette
										//Fait par Cédric Paquette Début
			...
			//Transfert DATA - FIN
				//SET EQUIPES
				$sql = "select count(*) as nbr, (select GROUP_CONCAT(ee.id_equipe) from entraineur_equipe ee where ee.id_entraineur = e.id_entraineur) as listID from entraineurs e inner join entraineur_equipe ee on e.id_entraineur = ee.id_entraineur where e.id_entraineur = ".$_GET['id'];
		...
		//if(($boo_checkSaison) and ($boo_checkNum) and ($boo_checkEQ) and( ($_POST['date_naissance'] == "") or (safeDate($_POST['date_naissance']))) ){
			if ($_GET['MODE'] == 'aj'){
				try {
					...
					if( ($_SESSION['AS'] == 'joueurs') and (!empty($_POST['nom']))and (!empty($_POST['prenom'])) ){
						...
						foreach ($_SESSION['arrayChamps'] as $key=>$value){
							if ($ctr < $_SESSION['ctr_champs']){
								...
								//Modification fait par Cédric Paquette Début
								...
								//Modification fait par Cédric Paquette Fin
								else {
									$sql2 = substr_replace($sql2,"`", strlen($sql2), 0);
						...
						$checkPhoto = false;
						$url_photo = "/Diablos_Archive/Diablos_en_fusion/Site/Images/default.png";
					...
					*/
						//ajout photo-joueurs
								if((isset($_POST['inputHid'])) and $_POST['inputHid'] != ""){
									...
									$conn->exec("update joueurs set photo_profil = '/Diablos_Archive/Diablos_en_fusion/Site/Images/".$_POST["inputHid"]."' where id_joueur = ".$_SESSION['max']);
									
								}
								else{
									$sql2 = substr_replace($sql2,'/Diablos_Archive/Diablos_en_fusion/Site/Images/default.png', strlen($sql2), 0);
									$conn->exec("update joueurs set photo_profil = '/Diablos_Archive/Diablos_en_fusion/Site/Images/default.png' where id_joueur = ".$_SESSION['max']);
								}
					/* fin */
								...
								for ($k = 0;$k<=$i;$k++){
									echo '<script>console.log("k='.$k.'");</script>';
										if(($k == 0) and (!empty($_POST['id_equipe'])) ){
											...
											$sql = "insert into `joueurs_equipes` (`id_joueur`,`id_equipe`,`id_position`,`numero`) VALUES (".$_SESSION['max'].",".$_POST['id_equipe'].",".$_POST['id_position'].",".$valueNum.")";
											$conn->exec($sql);
										}
										...
										elseif( ($k > 0) and (($_POST['id_equipe'.$k])!="") ){
											...
											echo '<script>console.log("ici3");</script>';
												...
												$sql = "insert into `joueurs_equipes` (`id_joueur`,`id_equipe`,`id_position`,`numero`) VALUES (".$_SESSION['max'].",".$_POST['id_equipe'.$k].",".$_POST['id_position'.$k].",".$valueNum.")";
												//echo '<script>console.log("'.$sql.'");</script>';
												$conn->exec($sql);
					...
					*/
					...
					/* fin */
							...
							if ($end > 0){
								for ($k = 0;$k<$i-1;$k++){
									if(($k == 0) and (!empty($_POST['id_equipe'])) ){
											...
											echo '<script>console.log("PASS")</script>';*/
										$sql = "insert into `entraineur_equipe` (`id_entraineur`,`id_equipe`,`role`) VALUES (".$maxID.",".$_POST['id_equipe'].",'".str_replace("'","''",$_POST['role'])."')";
										echo '<script>console.log("'.$sql.'")</script>';
										$conn->exec($sql);
									...
									elseif( ($k > 0) and (!empty($_POST['id_equipe'.$k])) ){
										...
										if ($nbrMatch == 0){
											...
											$sql = "insert into `entraineur_equipe` (`id_entraineur`,`id_equipe`,`role`) VALUES (".$maxID.",".$_POST['id_equipe'.$k].",'".str_replace("'","''",$_POST['role'.$k])."')";
											$conn->exec($sql);
											//move_uploaded_file($_FILES["photo_athlete".$k]["tmp_name"], "Profil/".$_FILES["photo_athlete".$k]["name"]);
						...
						//Entraineurs - FIN
						//MESSAGE
						$m_entraineur = "Entraineur ajouté avec succès.";
					...
					*/
						//ajout photo-equipe
						$path = "/Diablos_Archive/Diablos_en_fusion/Site/Images/default.png";
								if((isset($_POST['inputHid'])) and $_POST['inputHid'] != ""){
								    move_uploaded_file($_FILES["photo_athleteDefault"]["tmp_name"], "Images/".$_FILES["photo_athleteDefault"]["name"]);
									$path = "/Diablos_Archive/Diablos_en_fusion/Site/Images/".$_FILES["photo_athleteDefault"]["name"];
								}
					/* fin */	
						...
						$sql = substr_replace($sql,"','", strlen($sql), 0);
						$sql = substr_replace($sql,str_replace("'","''",$path), strlen($sql), 0);	
						$sql = substr_replace($sql,"')", strlen($sql), 0);
						echo '<script type="text/javascript">console.log("before");</script>';
					...
					//Équipe - FIN
					//MESSAGE
					$m_equipe = "Équipe ajoutée avec succès.";
				...
				if( (isset($_POST['prenom'])) and (isset($_POST['nom'])) and ( (empty($_POST['prenom'])) or (empty($_POST['nom'])) ) ){
					if( ($_GET['AS'] == 'aj') and ($_SESSION['joueurs']) ){
						$m_equipe = "Nom et prénom obligatoire.";
					}
					else{
						$m_equipe = "Nom et prénom obligatoire.";
					}
				...
				elseif( (isset($_POST['nom'])) and (isset($_POST['id_sport'])) and ( (empty($_POST['nom'])) or (empty($_POST['id_sport'])) ) ){
					$m_equipe = "Nom et sport obligatoire.";
					echo '<script>alert("'.$m_equipe.'");</script>';
					//echo '<script>setAlert(204,56,74,"'.$m_equipe.'");</script>';
					//echo '<script>setTimeout(function(){unsetAlert()},10000);</script>';
				}
			...
			elseif($_GET['MODE'] == 'mod'){
				try {
					...
					Adam Grenon
					*/
						//update photo-joueurs
								if((isset($_POST['inputHid'])) and $_POST['inputHid'] != ""){
								    move_uploaded_file($_FILES["photo_athleteDefault"]["tmp_name"], "Images/".$_FILES["photo_athleteDefault"]["name"]);
									$conn->exec("update joueurs set photo_profil = '/Diablos_Archive/Diablos_en_fusion/Site/Images/".$_POST["inputHid"]."' where id_joueur = ".$_GET['id']);
									
								}
								else{
									$sql2 = substr_replace($sql2,'/Diablos_Archive/Diablos_en_fusion/Site/Images/default.png', strlen($sql2), 0);
									$conn->exec("update joueurs set photo_profil = '/Diablos_Archive/Diablos_en_fusion/Site/Images/default.png' where id_joueur = ".$_GET['id']);
								}
					/* fin */
						//info personnelle
					...
					//UPDATE JOUEURS-EQUIPES - FIN
						//MESSAGE
						echo '<script>window.alert("Joueur modifié avec succès.");</script>';
					...
					*/
						//update photo-joueurs
								if((isset($_POST['inputHid'])) and $_POST['inputHid'] != ""){
								    move_uploaded_file($_FILES["photo_athleteDefault"]["tmp_name"], "Images/".$_FILES["photo_athleteDefault"]["name"]);
									$conn->exec("update entraineurs set photo_profil = '/Diablos_Archive/Diablos_en_fusion/Site/Images/".$_POST["inputHid"]."' where id_entraineur = ".$_GET['id']);
									
								}
								else{
									$sql2 = substr_replace($sql2,'/Diablos_Archive/Diablos_en_fusion/Site/Images/default.png', strlen($sql2), 0);
									$conn->exec("update entraineurs set photo_profil = '/Diablos_Archive/Diablos_en_fusion/Site/Images/default.png' where id_entraineur = ".$_GET['id']);
								}
					/* fin */
						//info personnelle
							$sql = "UPDATE `concep3t_diablos`.`personnes` SET ";
						...
						//UPDATE ENTRAINEURS-EQUIPES
							...
							$end = count($EonPage);
							//echo '<script>console.log("END = '.$end.'");</script>';
							$conn->exec("Delete from entraineur_equipe where id_entraineur = ".$_GET['id']);
							if ($end > 0){
								for ($k = 0;$k<$i-1;$k++){
									if(($k == 0) and (!empty($_POST['id_equipe'])) ){
										$sql = "insert into `entraineur_equipe` (`id_entraineur`,`id_equipe`,`role`) VALUES (".$_GET['id'].",".$_POST['id_equipe'].",'".str_replace("'","''",$_POST['role'])."')";
										$conn->exec($sql);
									}
									elseif( ($k > 0) and (!empty($_POST['id_equipe'.$k])) ){
										$stmt = new PDO("mysql:host=localhost;dbname=concep3t_diablos", "concep3t_admin", "admin");		
										...
										if ($nbrMatch == 0){
											$sql = "insert into `entraineur_equipe` (`id_entraineur`,`id_equipe`,`role`) VALUES (".$_GET['id'].",".$_POST['id_equipe'.$k].",'".str_replace("'","''",$_POST['role'.$k])."')";
											$conn->exec($sql);
										}
										else{
											$m_equipe = "L'entraineur ne peut être dans la même équipe deux fois";
						...
						//FIN ENTRAINEUR
						//MESSAGE
						echo '<script>window.alert("Entraineur modifié avec succès.");</script>';
						echo '<script language="javascript">location.assign("../../index.php")</script>';
					...
					*/
						...
						//update photo-equipe
								if((isset($_POST['inputHid'])) and $_POST['inputHid'] != ""){
								    move_uploaded_file($_FILES["photo_athleteDefault"]["tmp_name"], "Images/".$_FILES["photo_athleteDefault"]["name"]);
									$conn->exec("update equipes set photo_equipe = '/Diablos_Archive/Diablos_en_fusion/Site/Images/".$_POST["inputHid"]."' where id_equipe = ".$_GET['id']);
									
								}
								else{
									$sql2 = substr_replace($sql2,'/Diablos_en_fusion/Site/Images/default.png', strlen($sql2), 0);
									$conn->exec("update equipes set photo_equipe = '/Diablos_Archive/Diablos_en_fusion/Site/Images/default.png' where id_equipe = ".$_GET['id']);
								}
					/* fin */
						//info personnelle
						...
						//FIN - EQUIPE
							//MESSAGE
							echo '<script>window.alert("Équipe modifiée avec succès.");</script>';
							echo '<script language="javascript">location.assign("../../index.php")</script>';
							//echo '<script>setAlert(92,184,92,"'.$m_equipe.'");</script>';
					...
					elseif( (empty($_POST['nom'])) or (empty($_POST['id_sport'])) ){
						$m_equipe = "Nom et sport requis pour la modification d'une équipe.";
						echo '<script>alert("'.$m_equipe.'");</script>';
						//echo '<script>setAlert(204,56,74,"'.$m_equipe.'");</script>';
						//echo '<script>setTimeout(function(){unsetAlert()},10000);</script>';
					}
